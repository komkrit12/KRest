...
Public Class CC_ferpay
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay = 'แจ้งยกเลิก' Order by cust_id ASC"
        Else
            sql += " select * from vw_no_ferpay"
        End If
    ...
    Sub select_data()
        Dim sql As String = ""
        Dim dta As DataTable
        sql += " select * from vw_no_ferpay where status_ferpay = 'แจ้งยกเลิก' Order by cust_id ASC"
        dta = class_conn.selectbase(sql)
        Me.DataGridView1.DataSource = dta
    End Sub
    ...
    Sub cencel_dataall()
        ...
        sql += " UPDATE fer_payment SET"
        sql += " status_ferpay = 'ยกเลิก'"
        sql += " WHERE status_ferpay= 'แจ้งยกเลิก'"
        If class_conn.writebase(sql) = True Then
            select_data()
