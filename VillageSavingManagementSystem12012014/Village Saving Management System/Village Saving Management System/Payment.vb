...
Public Class Payment
    ...
    Sub insert_data()
        ...
        Dim sql As String = ""
        sql += " Insert into payment (contract_no,pay_value,balance,create_date,createby)"
        sql += " Values ('" + Me.txt_conid.Text + "'," + Me.txt_pay.Text + ""
        sql += " ," + Me.txt_balance.Text + ",getdate(),'" + Me.txt_create.Text + "' )"
    ...
    Sub check_balance()
        ...
        sql_con += " select * from contracts where cust_id ='" + Me.txt_id.Text + "'"
        ...
        If (count <> 0) Then
            ...
            sum_price = dta_con.Rows(0).Item("loan_amount").ToString
            con_id = dta_con.Rows(0).Item("contract_no").ToString
        Else
            sum = 0
            interest = 0
            sum_price = 0
        End If
        sql_pay += " select top 1 * from payment where contract_no ='" + con_id + "' order by pay_id desc"
    ...
    Private Sub btn_Search1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Search1.Click
        all_customer_pay.ShowDialog()
        If all_customer_pay.DialogResult = Windows.Forms.DialogResult.OK Then
