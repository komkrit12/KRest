...
Public Class Oil_Pay
    ...
    Sub insert_data()
        ...
        Dim sql As String = ""
        sql += " Insert into oil_payment (contract_oil_no,pay_value,balance,create_date)"
    ...
    Sub check_balance()
        ...
        sql_con += " select Top 1* from vw_oiloil where cust_id ='" + Me.txt_id.Text + "'   order by o_id desc"
        ...
        If (count <> 0) Then
            ...
            con_id = dta_con.Rows(0).Item("contract_oil_no").ToString
        Else
            sum = 0
            interest = 0
            sum_price = 0
        End If
        sql_pay += " select top 1 * from oil_payment where contract_oil_no ='" + con_id + "' order by pay_id desc"
    ...
    Private Sub Oil_Pay_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        username_log()
    ...
    Private Sub btn_Search1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Search1.Click
        all_customer_oilpay.ShowDialog()
        If all_customer_oilpay.DialogResult = Windows.Forms.DialogResult.OK Then
