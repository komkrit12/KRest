...
Public Class NC_customers
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from customers where cust_fname like '%" + Me.txt_search.Text + "%' and status is null Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from customers where cust_id like '%" + Me.txt_search.Text + "%' and status is null Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from customers where personal_id like '%" + Me.txt_search.Text + "%' and status is null Order by cust_id ASC"
        Else
