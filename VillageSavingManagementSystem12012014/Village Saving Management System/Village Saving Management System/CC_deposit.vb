...
Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_tarns = 'แจ้งยกเลิก' and trans_type = 'DEPOSIT' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_tarns = 'แจ้งยกเลิก' and trans_type = 'DEPOSIT' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_tarns = 'แจ้งยกเลิก' and trans_type = 'DEPOSIT' Order by cust_id ASC"
        Else
    ...
    Sub select_data()
        Dim sql As String = ""
        Dim dta As DataTable
        sql += " select * from vw_no_trans where status_tarns = 'แจ้งยกเลิก' and trans_type = 'DEPOSIT' Order by cust_id ASC"
        dta = class_conn.selectbase(sql)
        Me.DataGridView1.DataSource = dta
    End Sub
    ...
    Private Sub DataGridView1_CellContentClick_1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        Me.txt_deposit.Text = DataGridView1.Rows.Item(UserSelectRow).Cells("trans_value").Value.ToString
