...
Public Class NC_contracts
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_con is null Order by time_create DESC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_con is null Order by time_create DESC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_con is null Order by time_create DESC"
        Else
