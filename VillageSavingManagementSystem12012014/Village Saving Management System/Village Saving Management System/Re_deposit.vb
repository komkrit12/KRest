...
Public Class Re_deposit
    ...
    Sub select_data()
        ...
        sql_de += " select sum(cost_de) as sum_de from trans_vw1 "
        dta_de = class_conn.selectbase(sql_de)
        count_de = dta_de.Rows.Count
        If (count_de <> 0) Then
            deposit = dta_de.Rows(0).Item("sum_de").ToString
        Else
            deposit = 0
        End If

        sql_w += " select sum(cost_tran) as sum_wi from trans_vw2 "
        dta_w = class_conn.selectbase(sql_w)
        count_w = dta_w.Rows.Count
        If (count_w <> 0) Then
            Withdrawal = dta_w.Rows(0).Item("sum_wi").ToString
        Else
            Withdrawal = 0
        End If
        ...
        sql += " select * from vw_re_tran where time_create >= '" + date_from + " 00:00:00' and  time_create <= '" + date_to + " 23:59:59' and trans_type = 'DEPOSIT' "
        dta = class_conn.selectbase(sql)

        ReportViewer1.LocalReport.DataSources.Clear()
        ReportViewer1.LocalReport.ReportPath = "..\..\Report\re_deposit.rdlc"
        ReportViewer1.LocalReport.DataSources.Add(New ReportDataSource("DataSet1", dta))
        Dim myParam1 As New ReportParameter("sum_balance", cost)
    ...
    Private Sub Re_deposit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
