...
Public Class NC_withdrawal
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_tarns is null and trans_type = 'Withdrawal' Order by time_create DESC "
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_tarns is null and trans_type = 'Withdrawal' Order by time_create DESC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_trans where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_tarns is null and trans_type = 'Withdrawal' Order by time_create DESC"
        Else
