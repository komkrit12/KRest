...
Public Class CC_fercon
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_fercon where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_fercon = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_fercon where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_fercon = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_fercon where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_fercon = 'แจ้งยกเลิก' Order by cust_id ASC"
        Else
            sql += " select * from vw_no_fercon"
        End If
    ...
    Sub select_data()
        Dim sql As String = ""
        Dim dta As DataTable
        sql += " select * from vw_no_fercon where status_fercon = 'แจ้งยกเลิก' Order by cust_id ASC"
        dta = class_conn.selectbase(sql)
        Me.DataGridView1.DataSource = dta
    End Sub
    ...
    Sub cencel_dataall()
        ...
        sql += " UPDATE fer_contracts SET"
        sql += " status_fercon = 'ยกเลิก'"
        sql += " WHERE contract_fer_no= 'แจ้งยกเลิก'"
        If class_conn.writebase(sql) = True Then
            select_data()
    ...
    Private Sub DataGridView1_CellContentClick_1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        Me.txt_conid.Text = DataGridView1.Rows.Item(UserSelectRow).Cells("contract_fer_no").Value.ToString
