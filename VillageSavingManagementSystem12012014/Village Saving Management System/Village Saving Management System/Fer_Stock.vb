...
Public Class Fer_Stock
    ...
    Sub insert_data()
        ...
        Dim sql As String = ""
        sql += " Insert into fer_receive (fer_id,receive,price,sum_price,balance,number,receive_date,createby)"
    ...
    Sub check_fer()
        ...
        sql_con += " SELECT SUM(buy_amount) as sum_amount from vw_fer2 where fer_id = " + Me.cbo_type.SelectedValue.ToString
        ...
        sql_re += " select sum(receive) as sum_amount from vw_fer1 where fer_id = " + Me.cbo_type.SelectedValue.ToString
    ...
    Sub select_data()
        Dim sql As String = ""
        Dim dta As DataTable
        sql += " select Top 1 fer_type,receive,price,sum_price,number,receive_date,createby,balance from vw_ferstock ORDER BY receive_date DESC "
        dta = class_conn.selectbase(sql)
    ...
    Sub check_data()
        ...
        ElseIf Me.txt_price.Text = "0" Then
            MessageBox.Show("กรุณาเลือกสูตรปุ๋ยก่อนการบันทึก", "Village Saving Management System", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            insert_data()
    ...
    Private Sub Fer_Stock_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        data_combobox()
        username_log()
