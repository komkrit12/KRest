...
Public Class NC_ferpay
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay is null  Order by create_date DESC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay is null Order by create_date DESC "
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_ferpay where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_ferpay is null  Order by create_date DESC "
        Else
