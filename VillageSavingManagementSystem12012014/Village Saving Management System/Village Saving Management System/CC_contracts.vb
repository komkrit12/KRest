...
Public Class CC_contracts
    ...
    Sub load_data()
        ...
        If Me.cbo_search2.Text = "ชื่อ" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where cust_fname like '%" + Me.txt_search.Text + "%' and status is null and status_con = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "รหัสสมาชิก" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where cust_id like '%" + Me.txt_search.Text + "%' and status is null and status_con = 'แจ้งยกเลิก' Order by cust_id ASC"
        ElseIf Me.cbo_search2.Text = "เลขบัตรประชาชน" Then
            sql += " select *, cust_fname + ' ' + cust_lname AS FullName from vw_no_con where personal_id like '%" + Me.txt_search.Text + "%' and status is null and status_con = 'แจ้งยกเลิก' Order by cust_id ASC"
        Else
            sql += " select * from vw_no_con"
        End If
    ...
    Sub select_data()
        Dim sql As String = ""
        Dim dta As DataTable
        sql += " select * from vw_no_con where status_con = 'แจ้งยกเลิก' Order by cust_id ASC"
        dta = class_conn.selectbase(sql)
        Me.DataGridView1.DataSource = dta
    End Sub
    ...
    Sub cencel_dataall()
        ...
        sql += " UPDATE contracts SET"
        sql += " status_con = 'ยกเลิก'"
        sql += " WHERE status_con= 'แจ้งยกเลิก'"
        If class_conn.writebase(sql) = True Then
            select_data()
        ...
        Else
            MessageBox.Show("การยกเลิกล้มเหลว", "มีปัญหา", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    Private Sub DataGridView1_CellContentClick_1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        Me.txt_amount.Text = DataGridView1.Rows.Item(UserSelectRow).Cells("loan_amount").Value.ToString
