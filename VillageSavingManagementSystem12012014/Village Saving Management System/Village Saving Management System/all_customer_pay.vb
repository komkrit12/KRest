...
Public Class all_customer_pay
    ...
    Sub load_data()
        ...
        If Me.cbo_search.Text = "ชื่อ" Then
            sql += " select * from vw_con where cust_fname like '%" + Me.txt_search.Text + "%' and status_con is null and status is null order by cust_id asc"
        ElseIf Me.cbo_search.Text = "รหัสสมาชิก" Then
            sql += " select * from vw_con where cust_id like '%" + Me.txt_search.Text + "%' and status_con is null and status is null order by cust_id asc"
        ElseIf Me.cbo_search.Text = "เลขบัตรประชาชน" Then
            sql += " select * from vw_con where personal_id like '%" + Me.txt_search.Text + "%' and status_con is null and status is null order by cust_id asc"
    ...
    Private Sub DataGridView1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellContentClick
        ...
        strconid = DataGridView1.Rows.Item(UserSelectRow).Cells("contract_no").Value.ToString
        ...
        strloan = DataGridView1.Rows.Item(UserSelectRow).Cells("loan_amount").Value.ToString
        strtime1 = DataGridView1.Rows.Item(UserSelectRow).Cells("firstyear").Value.ToString
